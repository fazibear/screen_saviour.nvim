...
describe("integration", function()
  ...
  before_each(function()
    require("screen_saviour.manager").clean()
  ...
  it("unhandled error doesn't break next animations", function()
    ...
    require("screen_saviour").register_animation(test_animation)
    setup_viewport({ "aaaaa", "     " }, {})
    local ok, _ = pcall(vim.cmd, "ScreenSaviour test")
    assert.is_false(ok)
    vim.cmd("ScreenSaviour make_it_rain")
  end)
  ...
  it("raises an error if treesitter parser is missing", function()
    ...
    local ok, _ = pcall(vim.cmd, "ScreenSaviour make_it_rain")
    assert.is_false(ok)
  end)
end)
